    ...
    vector<int> asteroidCollision(vector<int>& asteroids) {
     vector<int> res;
        for(auto curr : asteroids){
            if(curr>0){
                res.push_back(curr);// will never collide no matter what
            }else{
                while(res.size()>0 && res.back()>0 && -curr>res.back())
                    res.pop_back();
                if(res.size() == 0 || res.back()<0)
                    res.push_back(curr);
                else if(res.size()>0 && -res.back() == curr)
                    res.pop_back();
            }
        }
        return res;
