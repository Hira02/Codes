...
//used find-union algorithm here 
int Solution::solve(int A, vector<vector<int> > &B) {
    
    sort(B.begin(), B.end(), comp);
    
    int parent[A]; // array will define the each nodes current parent value
    fill_n(parent, A, -1);
    
    int ans = 0;
    
    for(int i=0;i<B.size();i++) {
        int up = find(parent, B[i][0]-1); // find parent of A
        int vp = find(parent, B[i][1]-1); // find parent of B
