...
For example, after one cup of champagne is poured, the top most glass is full.  After two cups of champagne are poured,
 ...
 full - there are 3 full glasses total now.  After four cups of champagne are poured, the third row has the middle glass half full, 
...
Explanation: We poured 2 cups of champange to the top glass of the tower (which is indexed as (0, 0)).
 There is one cup of excess liquid. The glass indexed as (1, 0) and the glass indexed as (1, 1) will
...
{
public:
    void pourGlasses(int row, vector<double> &a, vector<double> &b)
    {
        for (int i = 0; i <= 100; i++)
        {
            b[i] = 0.0;
    ...
    double champagneTower(int poured, int query_row, int query_glass)
    {
        ...
        while (currRow <= query_row)
        {
            if (currRow % 2 == 1)
            {
                pourGlasses(currRow, prev, curr);
            }
            else
            {
                pourGlasses(currRow, curr, prev);
            }
            currRow++;
        }
        double res = (query_row % 2 == 1) ? curr[query_glass] : prev[query_glass];
        return min((double)res, 1.0);
    }
};
/*
Approach : Leetcode
Instead of keeping track of how much champagne should end up in a glass, keep track of the total amount of champagne that 
flows through a glass. For example, if poured = 10 cups are poured at the top, then the total flow-through of the top glass is 10;
 ...
 In general, if a glass has flow-through X, then Q = (X - 1.0) / 2.0 quantity of champagne will equally flow left and right.
  We can simulate the entire pour for 100 rows of glasses. A glass at (r, c) will have excess champagne flow towards (r+1, c) 
  and (r+1, c+1).
  */