...
class Solution {
public:
    unsigned getWidth(TreeNode* root, int level,unsigned nodeOrder, vector<pair<unsigned, unsigned>>& counter){
        if(root == NULL) return 0;
        if(counter.size() == level) counter.push_back({nodeOrder, nodeOrder});
        else
            counter[level].second = nodeOrder;
        return max({counter[level].second - counter[level].first + 1, getWidth(root->left, level+1, nodeOrder*2, counter), getWidth(root->right, level+1, nodeOrder*2+1, counter)});
    }
